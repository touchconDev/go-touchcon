...
RUN \
  echo 'geth init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.touchcon/keystore/ && cp /signer.json /root/.touch/keystore/' >> geth.sh && \{{end}}
...
// an Touchcon node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Touchcon node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.touchcon"],
