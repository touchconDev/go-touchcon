...
#
# The cluster contains a bootnode, a gtc node and multiple swarm nodes, with
...
# static gtc configuration
GTC_IP="192.168.33.3"
GTC_RPC_PORT="8545"
GTC_RPC_URL="http://${GTC_IP}:${GTC_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gtc and
# swarm nodes
start_bootnode() {
...
# start_gtc_node starts a gtc node with --datadir pointing at <base-dir>/gtc
# and a single, unlocked account with password "gtc"
start_gtc_node() {
  local dir="${base_dir}/gtc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gtc account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GTC_IP}"
    --rpcport   "${GTC_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gtc" "${GTC_IP}" "$(which gtc)" ${args[@]}
}
...
create_account() {
  ...
  gtc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
