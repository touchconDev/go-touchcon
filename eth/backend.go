...
import (
	...
	"github.com/touchconDev/go-touchcon/eth/filters"
	...
	"github.com/touchconDev/go-touchcon/miner"
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	log.Info("Initialised chain configuration eth/backend.go", "config", chainConfig)
	...
	if !config.SkipBcVersionCheck {
		...
		if bcVersion != core.BlockChainVersion && bcVersion != 0 {
			return nil, fmt.Errorf("Blockchain DB version mismatch (%d / %d). Run gtc upgradedb.\n", bcVersion, core.BlockChainVersion)
		}
...
func (s *Ethereum) StartMining(local bool) error {
    ...
    // 추후 삭제 해야됨
    if true {
        err := errors.New("We can not start mining.")
        log.Error("Cannot start mining without etherbase", "err", err)
        return fmt.Errorf("blocked services due to touchcon stabilization: %v", err)
    }
    //============임의로 생성한 코드 stop================
    eb, err := s.Etherbase()
    if err != nil {
